...
export class HomePage{
  ...
  for (const title of allTitles) {
   expect(title.toLowerCase().includes(suggestionWordsAllowed2) || title.toLowerCase().includes(suggestionWordsAllowed1)).toBe(true);
    ...
    async validateProductName(suggestionWordsAllowed1: string, suggestionWordsAllowed2: string){
      const productTitles = this.page.locator('.a-section.a-spacing-small.puis-padding-left-small')//.locator('.sg-col-inner').locator('.a-section').locator('.a-link-normal').locator('span');; 
       await expect(productTitles.first()).toBeVisible();
  ...
  for (const title of allTitles) {
   expect(title.toLowerCase().includes(suggestionWordsAllowed2) || title.toLowerCase().includes(suggestionWordsAllowed1)).toBe(true);
    ...
    async validatePriceOfSearchedProducts(minPrice: number, maxPrice: number){
        const productPrice = this.page.locator('.a-price-whole')
       await expect(productPrice.first()).toBeVisible();

  // Step 4: Validation
  const productPrices = await productPrice.allTextContents();
  for (const productPrice of productPrices) {
    expect(Number(productPrice)).toBeGreaterThan(minPrice);
    expect(Number(productPrice)).toBeLessThan(maxPrice);
  }
}

  async selectbrand(brandName: string){
    const brand = this.page.locator('#brandsRefinements').getByText(`${brandName}`)
    await brand.click()
    }
}